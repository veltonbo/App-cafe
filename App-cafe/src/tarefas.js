...
function atualizarTarefas() {
  ...
  (window.tarefas || []).forEach(function(t, index) {
    ...
    item.className = 'item';
    // Troca a cor da borda esquerda conforme prioridade
    let corBorda = '#4caf50'; // padrão verde
    ...
    if (t.eAplicacao) {
      detalhesAplicacao = ` (${t.tipo}) - ${t.dosagem}`;
    }
    item.innerHTML = `
      <span>${formatarDataBR(t.data)} - ${t.descricao}${detalhesAplicacao} - ${t.setor ? t.setor : ''}</span>
...
function abrirModalTarefa(editar = false, prioridade = 'Alta', setor = 'Setor 01', tipo = 'Adubo') {
  document.getElementById('modalTarefaBg').style.display = 'flex';
  document.getElementById('btnFlutuanteAddTarefa').style.display = 'none';
  // Prioridade
  const prioridadeTarefa = document.getElementById('prioridadeTarefa');
  if (prioridadeTarefa) {
    prioridadeTarefa.innerHTML = `
      <option value="Alta">Alta</option>
      <option value="Média">Média</option>
      <option value="Baixa">Baixa</option>
    `;
    prioridadeTarefa.value = prioridade;
  }
  // Setor (dinâmico)
  if (typeof atualizarSelectsSetor === 'function') atualizarSelectsSetor();
  const setorTarefa = document.getElementById('setorTarefa');
  if (setorTarefa) setorTarefa.value = setor;
  // Tipo de aplicação
  const tipoAplicacao = document.getElementById('tipoAplicacao');
  if (tipoAplicacao) tipoAplicacao.value = tipo;
