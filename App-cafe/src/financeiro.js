...
function atualizarFinanceiro() {
  ...
  chavesOrdenadas.forEach(chave => {
    ...
    if (grupo.aVencer.length) {
      ...
      // Seção A Vencer (sem destaque visual)
      ...
      sub.innerHTML = `<div style='font-weight:600;margin:0 0 8px 0;'>A vencer <span style="font-size:0.8em;font-weight:400;color:#888;margin-left:12px;">Total: ${formatarValorBR(totalMes)}</span></div>`;
  ...
  if (todosPagos.length) {
    ...
    // Seção Pagos (sem destaque visual)
    const subPagos = document.createElement('div');
    subPagos.style.background = '';
    subPagos.style.border = '';
    subPagos.style.borderRadius = '';
    subPagos.style.margin = '12px 0 32px 0';
    subPagos.style.padding = '10px 12px 8px 12px';
    subPagos.innerHTML = `<div style='font-weight:600;margin:0 0 8px 0;'>Pagos</div>`;
