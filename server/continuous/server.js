...
async function serveReactPreview(res,url){if(!(url.pathname==='/irrigacao-next'||url.pathname.startsWith('/irrigacao-next/')))return false;const base=path.join(DIST,'irrigacao-next');let relative=url.pathname.replace(/^\/irrigacao-next\/?/,'');if(!relative||!path.extname(relative))relative='index.html';const file=path.resolve(base,relative);if(!file.startsWith(path.resolve(base)+path.sep)&&file!==path.resolve(base))return false;if(await streamFile(res,file,{noStore:true}))return true;return streamFile(res,path.join(base,'index.html'),{noStore:true})}
...
const server=http.createServer(async(req,nativeRes)=>{
  ...
  try{
    ...
    if(await serveReactPreview(res,url))return;
