...
const app={
  ...
  activeView:'summary',autoMode:'automatic',loading:false,sseAbort:null
};
...
function initDays(){
  ...
  qsa('#dayPicker button').forEach(b=>b.addEventListener('click',()=>b.classList.toggle('active')));
}
...
async function saveSchedule(){
  ...
  try{
    ...
    toast('Programação salva e armada.');
    await loadDashboard();
...
async function saveAutomatic(){
  ...
  try{
    ...
    toast('Automático 2.0 atualizado.');await loadDashboard();
...
async function saveRain(){
  ...
  try{
    ...
    toast('Proteção por chuva atualizada.');await loadDashboard();
  }catch(e){toast(e.message)}
}
...
qsa('#autoMode button').forEach(b=>b.addEventListener('click',()=>{app.autoMode=b.dataset.mode;qsa('#autoMode button').forEach(x=>x.classList.toggle('active',x===b))}));
