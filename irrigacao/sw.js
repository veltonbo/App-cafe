...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{
      if(url.pathname==='/irrigacao/'||url.pathname==='/irrigacao'){
        const copy=response.clone();caches.open(CACHE).then(cache=>cache.put('/irrigacao/',copy)).catch(()=>null);
      }
      return response;
  ...
  if(url.origin===self.location.origin&&url.pathname.startsWith('/irrigacao/')){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{
      const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy)).catch(()=>null);return response;
